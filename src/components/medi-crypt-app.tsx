...
const decryptImageRubik = async (encryptedFile: Blob, providedKey: string): Promise<{ decryptedFile: Blob, decryptionTime: number, originalName: string, originalType: string }> => {
    ...
    if (keyStartIndex === -1) {
        throw new Error(`Invalid encrypted file: Missing '${keyMarker}' marker within the first ${metadataSearchLimit} bytes. The file might be corrupted or not generated by this application.`);
    }
