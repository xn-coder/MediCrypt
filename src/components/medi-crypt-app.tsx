...
const decryptImageRubik = async (encryptedFile: Blob, providedKey: string): Promise<{ decryptedFile: Blob, decryptionTime: number, originalName: string, originalType: string }> => {
    ...
    if (keyStartIndex === -1) {
        throw new Error("Invalid encrypted file: Missing '-key:' marker within the first " + metadataSearchLimit + " bytes.");
    }
  ...
  const handleDownload = (blob: Blob | null, defaultFilename: string) => {
    ...
    if (mode === 'decrypt' && decryptedFileInfo?.name) {
        ...
        if (parts.length > 1) {
         ...
         if (mimeType && knownExtensions[mimeType] && !filename.endsWith(knownExtensions[mimeType])) {
